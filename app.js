...
let chatMessages = []; // Chat messages list
let selectedAccent = 'indigo';
let syncTgTheme = true;
let isDarkTheme = false;

// Webhook URL for chat
...
let voicePlayingAudio = null; // tracks currently playing voice audio

function initChat() {
  const sendBtn = document.getElementById('chat-send-btn');
  const chatInput = document.getElementById('chat-input');
  const voiceBtn = document.getElementById('chat-voice-btn');
  const cancelRecordBtn = document.getElementById('recording-cancel-btn');
  const sendRecordBtn = document.getElementById('recording-send-btn');
  ...
  sendRecordBtn?.addEventListener('click', sendVoiceRecording);
...
async function sendToWebhook(msg) {
  try {
    ...
    // If voice message, attach base64 audio
    if (msg.type === 'voice' && msg.audioData) {
      payload.audio = msg.audioData;
      payload.audioDuration = msg.duration;
    }
    ...
    const found = chatMessages.find(m => m.id === msg.id);
    if (found) {
      found.status = response.ok ? 'sent' : 'error';
      saveState();
      renderChatMessages();
    }
  } catch (err) {
  ...
  if (chatMessages.length === 0) {
    container.innerHTML = `
      <div class="chat-welcome">
        ...
        <h2>Чат</h2>
        <p>Напишите сообщение или отправьте голосовое</p>
      </div>
    `;
    return;
  }
      ...
      html += `
        <div class="chat-msg ${msg.direction}">
          ...
          <div class="chat-msg-time">
            ${msg.time}
            ${msg.status === 'sending' ? ' <span class="sending-indicator">отправка...</span>' : ''}
            ${msg.status === 'error' ? ' <span class="send-error" onclick="retrySend(\'' + msg.id + '\')">⚠ повторить</span>' : ''}
          </div>
        </div>
      `;
    } else if (msg.type === 'voice') {
      const waveformBars = generateWaveformBars(msg.id);
      html += `
        <div class="chat-msg ${msg.direction}">
          <div class="chat-bubble">
            <div class="voice-msg-player">
              <button class="voice-play-btn" onclick="playVoiceMessage('${msg.id}')" id="voice-btn-${msg.id}">
                <svg class="play-icon" viewBox="0 0 24 24" fill="currentColor"><polygon points="5 3 19 12 5 21 5 3"/></svg>
              </button>
              <div class="voice-msg-info">
                <div class="voice-waveform" id="waveform-${msg.id}">${waveformBars}</div>
                <span class="voice-duration" id="voice-dur-${msg.id}">${msg.duration || '0:00'}</span>
              </div>
            </div>
          </div>
          <div class="chat-msg-time">
...
function generateWaveformBars(msgId) {
  // Pseudo-random but deterministic heights based on msgId hash
  let hash = 0;
  ...
  let bars = '';
  for (let i = 0; i < 24; i++) {
    const h = 4 + Math.abs(Math.sin(hash * (i + 1) * 0.3)) * 16;
    bars += `<div class="voice-waveform-bar" style="height:${h.toFixed(0)}px" data-idx="${i}"></div>`;
  }
  return bars;
}
...
async function startVoiceRecording() {
  try {
    ...
    // Show recording UI
    document.getElementById('chat-voice-recording').style.display = 'flex';
    document.getElementById('chat-input').style.display = 'none';
    document.getElementById('chat-send-btn').style.display = 'none';
    document.getElementById('chat-voice-btn').style.display = 'none';
...
function cancelVoiceRecording() {
  ...
  // Hide recording UI
  document.getElementById('chat-voice-recording').style.display = 'none';
  document.getElementById('chat-input').style.display = '';
  document.getElementById('chat-send-btn').style.display = '';
  document.getElementById('chat-voice-btn').style.display = '';
}
...
function playVoiceMessage(msgId) {
  ...
  const durEl = document.getElementById(`voice-dur-${msgId}`);
  const waveformEl = document.getElementById(`waveform-${msgId}`);

  // If already playing this message, pause it
  if (voicePlayingAudio && voicePlayingAudio.msgId === msgId) {
    voicePlayingAudio.pause();
    voicePlayingAudio = null;
    if (btn) btn.innerHTML = '<svg class="play-icon" viewBox="0 0 24 24" fill="currentColor"><polygon points="5 3 19 12 5 21 5 3"/></svg>';
    // Reset waveform bars
    resetWaveform(waveformEl);
    return;
  }
  ...
  if (voicePlayingAudio) {
    voicePlayingAudio.pause();
    const prevBtn = document.getElementById(`voice-btn-${voicePlayingAudio.msgId}`);
    if (prevBtn) prevBtn.innerHTML = '<svg class="play-icon" viewBox="0 0 24 24" fill="currentColor"><polygon points="5 3 19 12 5 21 5 3"/></svg>';
    const prevWaveform = document.getElementById(`waveform-${voicePlayingAudio.msgId}`);
    resetWaveform(prevWaveform);
  }

  const audio = new Audio(msg.audioData);
  audio.msgId = msgId;
  voicePlayingAudio = audio;
  ...
  audio.addEventListener('timeupdate', () => {
    ...
    animateWaveform(waveformEl, progress);
  ...
  audio.addEventListener('ended', () => {
    voicePlayingAudio = null;
    if (btn) btn.innerHTML = '<svg class="play-icon" viewBox="0 0 24 24" fill="currentColor"><polygon points="5 3 19 12 5 21 5 3"/></svg>';
    if (durEl) durEl.textContent = msg.duration;
    resetWaveform(waveformEl);
  });
...
function animateWaveform(waveformEl, progress) {
...
function resetWaveform(waveformEl) {
  if (!waveformEl) return;
  waveformEl.querySelectorAll('.voice-waveform-bar').forEach(b => b.classList.remove('played'));
}

// Keyboard dismissal on tap outside input fields
function initKeyboardDismissal() {
  document.addEventListener('touchstart', (e) => {
    const active = document.activeElement;
    if (active && (active.tagName === 'INPUT' || active.tagName === 'TEXTAREA')) {
      // Check if the tap is outside the active input
  ...
  // Prevent overscroll / pull-to-close in Telegram WebApp
  document.addEventListener('touchmove', (e) => {
    const appContent = document.querySelector('.app-content');
    const chatMsgs = document.getElementById('chat-messages');
    
    // Allow scrolling only inside scrollable containers
    if (e.target.closest('.app-content') || e.target.closest('.chat-messages') || e.target.closest('.trips-scroll') || e.target.closest('.categories-scroll') || e.target.closest('.modal-content-sheet')) {
